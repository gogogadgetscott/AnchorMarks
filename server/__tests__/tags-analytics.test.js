...
beforeAll(async () => {
  ...
  const user = { email: `analytics${unique}@example.com`, password: "password123" };
...
describe("Tag Analytics Endpoint", () => {
  it("returns usage stats and co-occurrence pairs", async () => {
    const res = await agent.get("/api/tags/analytics").set("X-CSRF-Token", csrfToken);
    ...
    const pairAlphaBeta = res.body.cooccurrence.find(
      (p) => (p.tag_name_a === "alpha" && p.tag_name_b === "beta") || (p.tag_name_a === "beta" && p.tag_name_b === "alpha"),
    );
    const pairBetaGamma = res.body.cooccurrence.find(
      (p) => (p.tag_name_a === "beta" && p.tag_name_b === "gamma") || (p.tag_name_a === "gamma" && p.tag_name_b === "beta"),
    );
