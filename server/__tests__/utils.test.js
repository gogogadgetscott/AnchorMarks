describe('server/utils.js', () => {
  describe('isPrivateAddress', () => {
    ...
    it('allows public IPs without DNS lookup', async () => {
      const { isPrivateAddress } = require('../utils');
      await expect(isPrivateAddress('http://93.184.216.34')).resolves.toBe(false);
    });
  ...
  describe('fetchFavicon', () => {
    it('returns null for non-http(s) bookmark URL', async () => {
      ...
      await expect(fetchFavicon('ftp://example.com', 'id', db, '/tmp', 'test')).resolves.toBeNull();
    });

    it('returns null for private targets in production (SSRF guard)', async () => {
      const { fetchFavicon } = require('../utils');
      const db = { prepare: () => ({ run: () => {} }) };
      await expect(fetchFavicon('http://127.0.0.1', 'id', db, '/tmp', 'production')).resolves.toBeNull();
    });
    ...
    it('returns cached favicon path when file already exists', async () => {
      ...
      const result = await fetchFavicon('https://example.com/some/path', 'bookmark-id', db, '/tmp/favicons', 'test');

      expect(result).toBe('/favicons/example_com.png');
      expect(db.prepare).toHaveBeenCalled();
      expect(updateRun).toHaveBeenCalledWith('/favicons/example_com.png', '/favicons/example_com.png', 'bookmark-id');
    ...
    it('returns null for invalid URL input', async () => {
      const { fetchFavicon } = require('../utils');
      const db = { prepare: () => ({ run: () => {} }) };
      await expect(fetchFavicon('not-a-url', 'id', db, '/tmp', 'development')).resolves.toBeNull();
    });
    ...
    it('falls back between sources and updates DB on success (mocked network)', async () => {
      ...
      const { fetchFavicon } = require('../utils');
      const result = await fetchFavicon('https://example.com/some/path', 'bookmark-id', db, '/tmp/favicons', 'development');
    ...
    it('returns the same in-flight promise for concurrent requests (queue)', async () => {
      ...
      const p1 = fetchFavicon('https://example.com/one', 'bookmark-id', db, '/tmp/favicons', 'development');
      const p2 = fetchFavicon('https://example.com/two', 'bookmark-id', db, '/tmp/favicons', 'development');
      ...
      expect(results).toEqual(['/favicons/example_com.png', '/favicons/example_com.png']);
