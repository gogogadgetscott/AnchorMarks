...
// Test database path
const TEST_DB_PATH = path.join(__dirname, '../../anchormarks-test-smart-org.db');
...
describe('Smart Organization API', () => {
  ...
  describe('GET /api/smart-collections/suggest', () => {
    ...
    it('should include activity-based collections', async () => {
      ...
      expect(res.status).toBe(200);
      const activityCollections = res.body.collections.filter(c => c.type === 'activity');
    ...
    it('should include domain-based collections', async () => {
      ...
      expect(res.status).toBe(200);
      const domainCollections = res.body.collections.filter(c => c.type === 'domain');
