...
export const AppProvider: React.FC<AppProviderProps> = ({
  ...
  const [currentView, setCurrentView] = useState<string>(
    initialState?.currentView || "bookmarks",
  );
  const [currentFolder, setCurrentFolder] = useState<string | null>(
    initialState?.currentFolder || null,
  );
  const [currentTag, setCurrentTag] = useState<string | null>(
    initialState?.currentTag || null,
  );
  const [currentCollection, setCurrentCollection] = useState<string | null>(
    initialState?.currentCollection || null,
  );
  ...
  const [selectedBookmarks, setSelectedBookmarks] = useState<Set<string>>(
    initialState?.selectedBookmarks || new Set(),
  );
  const [viewMode, setViewMode] = useState<"grid" | "list" | "compact">(
    initialState?.viewMode || "grid",
  );
  const [hideFavicons, setHideFavicons] = useState(
    initialState?.hideFavicons || false,
  );
  const [aiSuggestionsEnabled, setAiSuggestionsEnabled] = useState(
    initialState?.aiSuggestionsEnabled || false,
  );
  const [dashboardConfig, setDashboardConfig] = useState<DashboardWidget[]>(
    initialState?.dashboardConfig || [],
  );
  const [tagMetadata, setTagMetadata] = useState<Record<string, any>>(
    initialState?.tagMetadata || {},
  );
