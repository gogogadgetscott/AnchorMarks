...
let globalDispatcher: ((type: DialogType, message: string, options: any) => Promise<any>) | null = null;
...
export async function showPrompt(message: string, options?: PromptOptions): Promise<string | null> {
    if (globalDispatcher) return globalDispatcher("prompt", message, options);
    return window.prompt(message, options?.defaultValue);
}

export async function showTagPicker(options?: TagPickerOptions): Promise<string[] | null> {
...
export function ConfirmProvider({ children }: { children: ReactNode }) {
    ...
    const openDialog = useCallback((type: DialogType, message: string, options: any) => {
    ...
    const tagPicker = useCallback((options?: TagPickerOptions) => {
