...
function getColorForCount(count: number, minCount: number, maxCount: number): string {
    if (maxCount === minCount) return COUNT_GRADIENT_STOPS[Math.floor(COUNT_GRADIENT_STOPS.length / 2)];
...
function getFontSizeForCount(count: number, minCount: number, maxCount: number, fontSizes: number[]): number {
...
export function TagCloud() {
    ...
    const { setCurrentView, setFilterConfig } = useUI();
    ...
    useEffect(() => {
        ...
        setCanvasHeight(Math.max(300, window.innerHeight - headerHeight - legendReserve));
    ...
    const tags = useMemo(() => {
        return Object.keys(tagMetadata)
            .map((name) => ({ name, count: tagMetadata[name].count }))
    ...
    const { minCount, maxCount, totalUsages } = useMemo(() => {
        if (displayedTags.length === 0) return { minCount: 0, maxCount: 0, totalUsages: 0 };
    ...
    const fontSizes = useMemo(() => getBaseFontSizes(canvasHeight), [canvasHeight]);

    const handleTagClick = (tagName: string) => {
        setCurrentView("all");
        setFilterConfig({ tags: [tagName] });
    };
        ...
        return (
            <div className="tag-cloud-container">
                <div className="tag-cloud-empty">
                    ...
                    <h3>No Tags Yet</h3>
                    <p>Try organizing your bookmarks with <kbd>#tags</kbd> to see them visualized here</p>
                </div>
            </div>
        );
    }
        ...
        <div className="tag-cloud-container">
            <div className="tag-cloud-view">
                <div className="tag-cloud-header">
                    <div className="tag-cloud-title">
                        ...
                        <span className="tag-cloud-count">
                            {showAll ? tags.length : displayedTags.length} of {tags.length} {pluralize(tags.length, "tag", "tags")}
                        </span>
                    </div>
                ...
                <div
                    ...
                    style={{ minHeight: `${canvasHeight}px`, '--tag-cloud-canvas-height': `${canvasHeight}px` } as any}
                >
                    {displayedTags.map((tag, index) => {
                        ...
                        const rotation = Math.random() > 0.85 ? (Math.random() > 0.5 ? "3deg" : "-3deg") : "0deg";
