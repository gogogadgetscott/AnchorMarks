...
const FolderItem = memo<FolderItemProps>(({ folder, level, onSelect, onEdit, onDelete }) => {
    ...
    >
      ...
      <div className="folder-info">
        ...
        {folder.bookmark_count !== undefined && (
          <div className="folder-count">{folder.bookmark_count} bookmarks</div>
        )}
      </div>
...
export const FoldersView = memo(() => {
  ...
  const folderTree = useMemo(() => {
    const buildTree = (parentId: string | null = null, level: number = 0): Array<{ folder: Folder; level: number }> => {
