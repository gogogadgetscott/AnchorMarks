...
export function useSmartTags(url: string, enabled = true) {
  const [smartSuggestions, setSmartSuggestions] = useState<SmartTagSuggestion[]>([]);
  ...
  const fetchSuggestions = useCallback(async (urlToFetch: string) => {
  ...
  const debouncedFetch = useCallback((urlToFetch: string) => {
