...
import { saveSettings } from "@features/bookmarks/settings.ts";
...
export async function initTagAnalyticsWidgets(): Promise<void> {
    try {
        ...
        widgets.forEach((root: any) => {
            ...
            const metricSel = root.querySelector(".tag-analytics-metric") as HTMLSelectElement;
            const limitSel = root.querySelector(".tag-analytics-limit") as HTMLSelectElement;
            const pairSortSel = root.querySelector(".tag-analytics-pairsort") as HTMLSelectElement;
            const colorUsage = root.querySelector(".tag-analytics-color-usage") as HTMLInputElement;
            const colorClicks = root.querySelector(".tag-analytics-color-clicks") as HTMLInputElement;
            const colorFavorites = root.querySelector(
                ".tag-analytics-color-favorites",
            ) as HTMLInputElement;
            const colorPairs = root.querySelector(".tag-analytics-color-pairs") as HTMLInputElement;
            ...
            const legendFavorites = root.querySelector(".legend-favorites") as HTMLElement;
            ...
            if (colorUsage) {
                colorUsage.addEventListener("change", (e: any) => {
                    ensureColors();
                    (state.dashboardWidgets[idx] as any).settings.colors.usage = e.target.value;
                    saveSettings({ dashboard_widgets: state.dashboardWidgets });
                    initTagAnalyticsWidgets();
                });
            }
            if (colorClicks) {
                colorClicks.addEventListener("change", (e: any) => {
                    ensureColors();
                    (state.dashboardWidgets[idx] as any).settings.colors.clicks = e.target.value;
                    saveSettings({ dashboard_widgets: state.dashboardWidgets });
                    initTagAnalyticsWidgets();
                });
            }
            ...
            if (colorPairs) {
                colorPairs.addEventListener("change", (e: any) => {
                    ensureColors();
                    (state.dashboardWidgets[idx] as any).settings.colors.pairs = e.target.value;
                    saveSettings({ dashboard_widgets: state.dashboardWidgets });
                    initTagAnalyticsWidgets();
                });
            }
    ...
    if (widget.type === "folder") {
        ...
        if (state.includeChildBookmarks) {
            ...
            folderBookmarks = state.bookmarks.filter((b) =>
                b.folder_id && allIds.includes(b.folder_id),
            );
        } else {
...
export function initDashboardDragDrop(): void {
    ...
    document.querySelectorAll(".dashboard-widget-freeform").forEach((widget: any) => {
    ...
    document.querySelectorAll('[data-action="remove-widget"]').forEach((btn: any) => {
...
function handleMouseMove(e: MouseEvent): void {
    if (state.isDraggingWidget && state.draggedWidget) {
        ...
        // Apply snap-to-grid
        (state.resizingWidget as HTMLElement).style.width = `${snapToGrid(newWidth)}px`;
        (state.resizingWidget as HTMLElement).style.height = `${snapToGrid(newHeight)}px`;
    }
}
...
function handleMouseUp(_e: MouseEvent): void {
    if (state.isDraggingWidget && state.draggedWidget) {
        ...
        const index = parseInt((state.draggedWidget as HTMLElement).dataset.widgetIndex || "-1");
        ...
        const index = parseInt((state.resizingWidget as HTMLElement).dataset.widgetIndex || "-1");
        if (state.dashboardWidgets[index]) {
...
export function addDashboardWidget(type: 'folder' | 'tag' | 'tag-analytics', id: string, x: number, y: number): void {
