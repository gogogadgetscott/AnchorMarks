...
export function renderFolders(): void {
        ...
        return folderList
            .map((f) => {
                ...
                const count = state.bookmarks.filter((b) =>
                    b.folder_id && allIds.has(b.folder_id),
                ).length;
    ...
    container.querySelectorAll(".folder-item").forEach((item: any) => {
        item.addEventListener("click", (e: any) => {
            ...
            updateActiveNav();
            import("@features/bookmarks/search.ts").then(({ renderActiveFilters }) => renderActiveFilters());
...
export function updateFolderParentSelect(currentId: string | null = null): void {
...
// Create folder
export async function createFolder(data: any, options: { closeModal?: boolean } = {}): Promise<any> {
...
export function editFolder(id: string): void {
    ...
    const parentSelect = document.getElementById("folder-parent") as HTMLSelectElement;
...
export async function renderFoldersForFilter(container: HTMLElement): Promise<void> {
    if (!container) return;
    ...
    const buildFolderTree = (parentId: string | null = null, level = 0): string => {
        return folders
