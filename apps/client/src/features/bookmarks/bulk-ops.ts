...
export async function bulkMove(): Promise<void> {
    const select = document.getElementById("bulk-move-select") as HTMLSelectElement;
    if (!select) return;
...
export async function bulkAddTags(): Promise<void> {
    ...
    const { clearSelections, renderBookmarks } = await import("@features/bookmarks/bookmarks.ts");
