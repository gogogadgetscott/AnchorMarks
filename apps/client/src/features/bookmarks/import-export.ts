...
import { loadFolders } from "@features/bookmarks/folders.ts";
...
function setImportProgress(statusState: 'start' | 'success' | 'error' | 'idle', message = ""): void {
...
function setDashboardImportProgress(statusState: 'start' | 'success' | 'error' | 'idle', message = ""): void {
    const statusEl = document.getElementById("import-dashboard-views-progress");
    const btn = document.getElementById("import-dashboard-views-btn") as HTMLButtonElement;
    if (!statusEl || !btn) return;
