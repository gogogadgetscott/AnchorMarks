...
import { loadFolders } from "@features/bookmarks/folders.js";
...
export async function exportHtml() {
  try {
    ...
    if (response.status === 401) {
      const { logout } = await import("@features/auth/auth.js");
      logout();
...
export async function resetBookmarks() {
  ...
  try {
    ...
    const { updateActiveNav } = await import("@utils/ui-helpers.js");
    updateActiveNav();
    closeModals();
    showToast(
