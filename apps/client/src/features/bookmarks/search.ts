...
export function sidebarFilterTag(tag: string): void {
    ...
    updateActiveNav();
    renderActiveFilters();
    import("@features/bookmarks/bookmarks.ts").then(({ renderBookmarks }) => renderBookmarks());
    renderSidebarTags();
}
    ...
    tags.forEach((tag) => {
        ...
        div.addEventListener("click", (e) => {
            ...
            if (state.currentView === "dashboard") {
                import("@features/bookmarks/dashboard.ts").then(({ addDashboardWidget }) => {
...
export function clearAllFilters(): void {
    ...
    updateActiveNav();
    renderActiveFilters();
    import("@features/bookmarks/bookmarks.ts").then(({ loadBookmarks }) => loadBookmarks());
    renderSidebarTags();
}
...
export function renderActiveFilters(): void {
    ...
    const collectionName = currentCollection
        ? (state as any).collections.find((c: any) => c.id === currentCollection)?.name
        : null;
...
export function toggleFilterTag(tag: string): void {
    if (state.filterConfig.tags.includes(tag)) {
        state.setFilterConfig({ ...state.filterConfig, tags: state.filterConfig.tags.filter((t) => t !== tag) });
    } else {
        state.setFilterConfig({ ...state.filterConfig, tags: [...state.filterConfig.tags, tag] });
    }
...
export function updateTagRenameUndoButton(): void {
    const btn = document.getElementById("tag-rename-undo-btn") as HTMLButtonElement;
    if (!btn) return;
...
export async function renderTagsForFilter(container: HTMLElement): Promise<void> {
    if (!container) return;
...
// Create a new tag
export async function createNewTag(name: string, color?: string): Promise<boolean> {
    if (!name || !name.trim()) {
