...
export async function showFilterDropdown(): Promise<void> {
    ...
    try {
        ...
        const sortSelect = document.getElementById("filter-sort-select") as HTMLSelectElement;
        if (sortSelect)
            sortSelect.value = state.filterConfig.sort || "recently_added";

        const tagModeSelect = document.getElementById("filter-tag-mode") as HTMLSelectElement;
...
function getFolderBookmarkCount(folderId: string | null): number {
    ...
    return state.bookmarks.filter((b) => b.folder_id && descendantIds.includes(b.folder_id))
        .length;
}
...
async function renderFoldersInDropdown(): Promise<void> {
    ...
    const renderFolderTree = (parentId: string | null = null, depth = 0) => {
        ...
        folderList.sort((a, b) => (1 /* folder.position is missing in the interface */) - (1));
...
function attachFilterDropdownListeners(): void {
    ...
    const folderSearchInput = document.getElementById("filter-folders-search") as HTMLInputElement;
    ...
    const tagSearchInput = document.getElementById("filter-tags-search") as HTMLInputElement;
    if (tagSearchInput) {
