...
function upsertUserSettings(db, userId, body = {}) {
  ...
  if (existing) {
    ...
    pushIf('ai_suggestions_enabled', body.ai_suggestions_enabled, (v) => (v ? 1 : 0));
    pushIf('theme', body.theme);
    pushIf('dashboard_mode', body.dashboard_mode);
    pushIf('dashboard_tags', body.dashboard_tags, (v) => (v ? JSON.stringify(v) : null));
    pushIf('dashboard_sort', body.dashboard_sort);
    pushIf('widget_order', body.widget_order, (v) => (v ? JSON.stringify(v) : null));
    pushIf('dashboard_widgets', body.dashboard_widgets, (v) => (v ? JSON.stringify(v) : null));
    pushIf('collapsed_sections', body.collapsed_sections, (v) => (v ? JSON.stringify(v) : null));
    pushIf('include_child_bookmarks', body.include_child_bookmarks);
    if (body.snap_to_grid !== undefined) pushIf('snap_to_grid', body.snap_to_grid ? 1 : 0);
    if (body.current_view !== undefined) pushIf('current_view', body.current_view);

    if (updates.length > 0) {
      updates.push('updated_at = CURRENT_TIMESTAMP');
      values.push(userId);
      return db.prepare(`UPDATE user_settings SET ${updates.join(', ')} WHERE user_id = ?`).run(...values);
    }
    return null;
  }
  ...
  return db
    ...
    .run(
      ...
      body.hide_sidebar || 0,
      body.ai_suggestions_enabled !== undefined && body.ai_suggestions_enabled !== null ? (body.ai_suggestions_enabled ? 1 : 0) : 1,
      body.theme || 'light',
...
module.exports = { applyDashboardConfigToUser, getUserSettings, upsertUserSettings };
