...
function updateFolder(db, id, userId, fields) {
  ...
  `).run(fields.name, fields.parent_id, fields.color, fields.icon, fields.position, id, userId);
}

function deleteFolder(db, id, userId) {
  db.prepare('UPDATE bookmarks SET folder_id = NULL WHERE folder_id = ? AND user_id = ?').run(id, userId);
  return db.prepare('DELETE FROM folders WHERE id = ? AND user_id = ?').run(id, userId);
}
...
function createFolder(db, id, userId, name, color, icon, position) {
  return db.prepare('INSERT INTO folders (id, user_id, name, color, icon, position) VALUES (?, ?, ?, ?, ?, ?)').run(id, userId, name, color, icon, position);
}

function findFolderByNameAndParent(db, userId, name, parentId) {
  return db.prepare('SELECT id FROM folders WHERE user_id = ? AND name = ? AND (parent_id = ? OR (? IS NULL AND parent_id IS NULL))').get(userId, name, parentId || null, parentId || null);
}

function getMaxPositionForParent(db, userId, parentId) {
  const row = db.prepare('SELECT MAX(position) as max FROM folders WHERE user_id = ? AND (parent_id = ? OR (? IS NULL AND parent_id IS NULL))').get(userId, parentId || null, parentId || null);
...
module.exports = { listFolders, getFolderById, updateFolder, deleteFolder, deleteAllForUser, createFolder };
