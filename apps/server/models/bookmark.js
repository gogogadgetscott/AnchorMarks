...
function listBookmarks(db, userId, opts = {}) {
  const { folder_id, include_children, favorites, search, tags, sort, limit, offset } = opts;
  ...
  if (search) {
    query += ' AND (b.title LIKE ? OR b.url LIKE ? OR b.description LIKE ? OR tg.tags_joined LIKE ?)';
    countQuery += ' AND (b.title LIKE ? OR b.url LIKE ? OR b.description LIKE ? OR tg.tags_joined LIKE ?)';
  ...
  if (sort) {
    switch ((String(sort)).toLowerCase()) {
      case 'recently_added': orderClause = ' ORDER BY created_at DESC'; break;
      case 'oldest_first': orderClause = ' ORDER BY created_at ASC'; break;
      case 'most_visited': orderClause = ' ORDER BY click_count DESC, created_at DESC'; break;
      case 'a_z': case 'a-z': orderClause = ' ORDER BY title COLLATE NOCASE ASC'; break;
      case 'z_a': case 'z-a': orderClause = ' ORDER BY title COLLATE NOCASE DESC'; break;
      default: orderClause = ' ORDER BY position, created_at DESC';
    }
  }
  query += orderClause;
...
function getBookmarkById(db, userId, id) {
  return db.prepare(`SELECT b.*, COALESCE(tg.tags_joined, '') as tags, COALESCE(tg.tags_detailed, '[]') as tags_detailed ${_baseSelect()} AND b.id = ?`).get(userId, userId, id);
}
...
function createBookmark(db, userIdOrData, maybeData) {
  ...
  const maxPos = db.prepare('SELECT MAX(position) as max FROM bookmarks WHERE user_id = ?').get(userId);
  const position = data.position != null ? data.position : ((maxPos && maxPos.max) || 0) + 1;
  ...
  db.prepare(`INSERT INTO bookmarks (id, user_id, folder_id, title, url, description, favicon, position, content_type) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`)
    .run(id, userId, folder_id, title, url, description, faviconUrl, position, content_type);

  return getBookmarkById(db, userId, id);
}

function updateBookmark(db, userId, id, fields = {}) {
  const { title, url, description, folder_id, is_favorite, position, favicon, tags, tag_colors } = fields;
  db.prepare(`
  ...
  `).run(title, url, description, folder_id, is_favorite, position, favicon, id, userId);

  if (tags !== undefined) {
    const tagHelpers = require('../tag-helpers');
    if (tags && tags.trim && tags.trim()) {
      const result = tagHelpers.ensureTagsExist(db, userId, tags, { returnMap: true });
      const overrides = require('../helpers/tags').normalizeTagColorOverrides(tag_colors || null, result.tagMap);
      tagHelpers.updateBookmarkTags(db, id, result.tagIds, { colorOverridesByTagId: overrides });
    } else {
...
function setThumbnailLocal(db, bookmarkId, localPath) {
  return db.prepare('UPDATE bookmarks SET thumbnail_local = ? WHERE id = ?').run(localPath, bookmarkId);
}

function findBookmarkIdByUrl(db, userId, url) {
  const row = db.prepare('SELECT id FROM bookmarks WHERE url = ? AND user_id = ?').get(url, userId);
  return row ? row.id : null;
}

function listUrls(db, userId) {
  return db.prepare('SELECT url FROM bookmarks WHERE user_id = ?').all(userId).map(r => r.url);
}
...
function incrementClick(db, a, b) {
  ...
  return db.prepare(`UPDATE bookmarks SET click_count = click_count + 1, last_clicked = CURRENT_TIMESTAMP WHERE id = ? AND user_id = ?`).run(id, userId);
}

module.exports = { listBookmarks, getBookmarkById, createBookmark, updateBookmark, deleteBookmark, incrementClick };
module.exports = Object.assign(module.exports, { deleteAllForUser, setThumbnailLocal, findBookmarkIdByUrl, listUrls, getSampleForSuggestion, getRecentCountForDomain, getMostClickedForDomain });
