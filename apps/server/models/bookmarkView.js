...
function createBookmarkView(db, userId, name, config) {
  const id = uuidv4();
  db.prepare(
    "INSERT INTO bookmark_views (id, user_id, name, config) VALUES (?, ?, ?, ?)",
  ).run(id, userId, name, JSON.stringify(config));
...
function updateBookmarkView(db, id, userId, name, config) {
  ...
  if (!view) return null;
  db.prepare(
    "UPDATE bookmark_views SET name = ?, config = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ? AND user_id = ?",
  ).run(
    name || view.name,
