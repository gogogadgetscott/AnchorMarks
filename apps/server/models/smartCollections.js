...
function listCollections(db, userId) {
  return db
    .prepare('SELECT * FROM smart_collections WHERE user_id = ? ORDER BY position')
    .all(userId);
}
...
function createCollection(db, userId, { name, icon, color, filters }) {
  const id = uuidv4();
  const maxPos = db
    .prepare('SELECT MAX(position) as max FROM smart_collections WHERE user_id = ?')
    .get(userId);
  const position = (maxPos.max || 0) + 1;

  db.prepare(
    'INSERT INTO smart_collections (id, user_id, name, icon, color, filters, position) VALUES (?, ?, ?, ?, ?, ?, ?)'
  ).run(id, userId, name, icon || 'filter', color || '#6366f1', JSON.stringify(filters), position);

  return db.prepare('SELECT * FROM smart_collections WHERE id = ?').get(id);
}

function updateCollection(db, id, userId, { name, icon, color, filters, position }) {
  db.prepare(
    `
  ...
  `,
  ).run(name, icon, color, filters ? JSON.stringify(filters) : null, position, id, userId);

  return db.prepare('SELECT * FROM smart_collections WHERE id = ?').get(id);
}

function deleteCollection(db, id, userId) {
  return db.prepare('DELETE FROM smart_collections WHERE id = ? AND user_id = ?').run(id, userId);
}
