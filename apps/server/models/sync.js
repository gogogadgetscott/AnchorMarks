      ...
      try {
        ...
        if (existing) {
          ...
          db.prepare('INSERT INTO folders (id, user_id, name, color, parent_id) VALUES (?, ?, ?, ?, ?)')
            .run(folder.id || uuidv4(), userId, folder.name, folder.color || '#6366f1', folder.parent_id);
          results.created++;
        }
      } catch (err) {
    ...
    for (const bm of bookmarks) {
      try {
        ...
        if (existing) {
          ...
          if (bm.tags) {
            const tagsString = Array.isArray(bm.tags) ? bm.tags.join(',') : bm.tags;
            const tagIds = require('../tag-helpers').ensureTagsExist(db, userId, tagsString);
            require('../tag-helpers').updateBookmarkTags(db, existing.id, tagIds);
          }
          results.updated++;
        } else {
          const id = uuidv4();
          ...
          db.prepare('INSERT INTO bookmarks (id, user_id, folder_id, title, url, favicon) VALUES (?, ?, ?, ?, ?, ?)')
            .run(id, userId, bm.folder_id, bm.title || bm.url, bm.url, faviconUrl);

          if (bm.tags) {
            const tagsString = Array.isArray(bm.tags) ? bm.tags.join(',') : bm.tags;
            const tagIds = require('../tag-helpers').ensureTagsExist(db, userId, tagsString);
            require('../tag-helpers').updateBookmarkTags(db, id, tagIds);
          }
