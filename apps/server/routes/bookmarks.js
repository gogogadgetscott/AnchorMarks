module.exports = function setupBookmarksRoutes(app, db, helpers = {}) {
  const { authenticateTokenMiddleware, validateCsrfTokenMiddleware, fetchFaviconWrapper, config } = helpers;
  ...
  app.get('/api/bookmarks', authenticateTokenMiddleware, (req, res) => {
    try {
      const { folder_id, search, favorites, tags, sort, limit, offset, include_children } = req.query;
      const opts = { folder_id, include_children: include_children === 'true', favorites, search, tags, sort, limit, offset };
  ...
  app.post('/api/bookmarks/fetch-metadata', authenticateTokenMiddleware, async (req, res) => {
    ...
    try {
      ...
      if (config.NODE_ENV === 'production' && (await require('../utils').isPrivateAddress(url))) {
        return res.status(403).json({ error: 'Cannot fetch metadata from private addresses' });
      }

      const metadata = await require('../app').fetchUrlMetadata ? await require('../app').fetchUrlMetadata(url) : await (async () => { return { title: new URL(url).hostname, description: '', url }; })();
      res.json(metadata);
    } catch (err) {
      console.error('Metadata fetch error:', err.message);
      try { res.json({ title: new URL(url).hostname, description: '', url }); } catch (e) { res.status(500).json({ error: 'Failed to fetch metadata', message: err.message }); }
    }
  });
    ...
    try {
      ...
      if (fields.tags !== undefined) {
        if (fields.tags && fields.tags.trim && fields.tags.trim()) {
          const tagResult = tagHelpers.ensureTagsExist(db, req.user.id, fields.tags, { returnMap: true });
          const overrides = require('../helpers/tags').normalizeTagColorOverrides(fields.tag_colors || fields.tagColorOverrides, tagResult.tagMap);
          tagHelpers.updateBookmarkTags(db, req.params.id, tagResult.tagIds, { colorOverridesByTagId: overrides });
        } else {
