module.exports = function setupDashboardRoutes(app, db, helpers = {}) {
  ...
  app.post('/api/dashboard/views', authenticateTokenMiddleware, (req, res) => {
    try {
      ...
      const view = dashboardModel.createDashboardView(db, req.user.id, name, config);
  ...
  app.put('/api/dashboard/views/:id', authenticateTokenMiddleware, (req, res) => {
    try {
      ...
      const view = dashboardModel.updateDashboardView(db, req.params.id, req.user.id, { name, config, position });
      res.json({ ...view, config: JSON.parse(view.config) });
    } catch (err) {
  ...
  app.post('/api/dashboard/views/:id/restore', authenticateTokenMiddleware, (req, res) => {
    try {
      ...
      if (!view || view.user_id !== req.user.id) return res.status(404).json({ error: 'View not found' });
