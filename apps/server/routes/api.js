...
function setupApiRoutes(app, db, helpers) {
  ...
  setupFoldersRoutes(app, db, { authenticateTokenMiddleware, validateCsrfTokenMiddleware });

  // Tags routes (migrated into controller)
  const { setupTagsRoutes } = require('../controllers/tags');
  setupTagsRoutes(app, db, { authenticateTokenMiddleware, validateCsrfTokenMiddleware });
  ...
  app.get("/api/tags/suggest-ai", authenticateTokenMiddleware, async (req, res) => {
    ...
    try {
      const userTags = db.prepare("SELECT DISTINCT name FROM tags WHERE user_id = ?").all(req.user.id).map((row) => row.name);
      const aiConfig = config.getAIConfig ? config.getAIConfig() : { provider: "none" };
      const suggestions = await aiTags.suggestTagsAI({ url, title: null, limit: parseInt(limit), userTags }, aiConfig);

      res.json({ suggestions, info: { provider: aiConfig.provider, model: aiConfig.model || null } });
    } catch (err) {
      if (err && (err.code === "AI_NOT_CONFIGURED" || err.code === "AI_KEY_MISSING" || err.code === "AI_UNSUPPORTED")) {
