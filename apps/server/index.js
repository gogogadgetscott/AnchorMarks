...
server.listen(config.PORT, config.HOST, () => {
    ...
    if (config.SSL_ENABLED) {
        ...
        lines.push(`SSL Key:                  ${config.SSL_KEY || "(not set)"} ${sslKeyExists ? "✓" : "✗"}`);
        lines.push(`SSL Cert:                 ${config.SSL_CERT || "(not set)"} ${sslCertExists ? "✓" : "✗"}`);
    }


    const dbExists = config.DB_PATH && fs.existsSync(config.DB_PATH);
    lines.push(`Database:                 ${config.DB_PATH || "(not set)"} ${dbExists ? "✓" : "✗"}`);
    lines.push(`Background jobs:          ${config.ENABLE_BACKGROUND_JOBS ? "enabled" : "disabled"}`);
    lines.push(`Favicon background jobs:  ${config.ENABLE_FAVICON_BACKGROUND_JOBS ? "enabled" : "disabled"}`);
