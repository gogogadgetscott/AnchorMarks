            ...
            if (bookmark && bookmark.url) {
                ...
                if (captureScreenshotFn) {
                    try {
                        ...
                        if (result.success) {
                            console.log(`[MetadataQueue] Thumbnail captured for ${bookmarkId}`);
                        } else if (result.error && result.error !== "Thumbnail generation is disabled") {
                            console.warn(`[MetadataQueue] Thumbnail failed for ${bookmarkId}: ${result.error}`);
                        }
                    } catch (thumbnailErr) {
                        console.error(
