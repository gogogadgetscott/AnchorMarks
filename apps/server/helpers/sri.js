    ...
    protocol
      .get(url, (response) => {
        if (
          ...
          // Follow redirect
          return fetchContent(response.headers.location)
            .then(resolve)
            .catch(reject);
        }
...
if (require.main === module) {
  ...
  switch (command) {
    case "generate":
      ...
      generateHashForUrl(args[0], args[1] || "sha384")
        .then((hash) => {
          ...
          console.log("\nHTML Example:");
          console.log(
            `<script src="${args[0]}" integrity="${hash}" crossorigin="anonymous"></script>`,
          );
        })
        .catch((err) => {
    ...
    default:
      ...
      console.log(
        "  validate <url> <hash>       Validate a URL against expected hash",
      );
