...
import { u as We, e as k } from "./bookmarks-hgxSTBhD.js";
import { s as w, a as _e } from "./ui-DJeZlV46.js";
const ge = 20;
function O(e) {
  return Me ? Math.round(e / ge) * ge : e;
}
function dt() {
  const e = document.body.classList.toggle("fullscreen-mode"),
  ...
  t && s && (t.classList.toggle("hidden", e), s.classList.toggle("hidden", !e));
}
function He() {
...
function Se() {
  const e = document.getElementById("dashboard-unsaved-indicator");
  e && e.classList.toggle("hidden", !xe);
}
function j(e) {
  const t = document.getElementById("dashboard-view-name");
  t &&
    (e
...
async function Ne() {
  ...
  if (e) {
    if (
      ...
      !document.getElementById("dashboard-views-btn"))
    ) {
      const t = document.createElement("button");
      ((t.id = "dashboard-views-btn"),
...
async function Ue() {
  ...
  (e.length === 0
    ...
    t.querySelectorAll(".view-item").forEach((a) => {
      const i = a.dataset.viewId,
        o = a.querySelector(".view-name"),
        d = o?.textContent || null,
        r = a.querySelector(".delete-view-btn");
      (o &&
        o.addEventListener("click", async (g) => {
          (g.preventDefault(), g.stopPropagation(), await U(i, d));
        }),
        r &&
          r.addEventListener("click", async (g) => {
            (g.preventDefault(), g.stopPropagation(), await N(i));
          }));
    }));
  const n = t.querySelector("#save-view-btn");
  (n &&
    n.addEventListener("click", async (a) => {
      (a.preventDefault(), a.stopPropagation(), await F());
    }),
    setTimeout(() => {
...
function Le(e) {
  const t = document.getElementById("views-dropdown");
  t &&
    !t.contains(e.target) &&
    e.target.id !== "dashboard-views-btn" &&
    (t.remove(), document.removeEventListener("click", Le));
}
async function F() {
  const e = prompt("Enter a name for this view:");
  if (e)
    try {
      const t = {
          dashboard_mode: $.mode,
          dashboard_tags: $.tags,
          dashboard_sort: $.bookmarkSort,
          widget_order: ze,
          dashboard_widgets: l,
          include_child_bookmarks: pe ? 1 : 0,
        },
        s = await T("/dashboard/views", {
          method: "POST",
          body: JSON.stringify({ name: e, config: t }),
        });
      (w("View saved!", "success"),
        document.getElementById("views-dropdown")?.remove(),
        we(s.id),
        be(e),
        j(e),
        Ee());
      const { saveSettings: n } = await f(
        async () => {
          const { saveSettings: a } = await import("./settings-B0ZpC5LD.js");
      ...
      if (
        (await n({
          current_dashboard_view_id: s.id,
          current_dashboard_view_name: e,
        }),
        confirm("Create a bookmark shortcut for this view?"))
      ) {
        const { createBookmark: a } = await f(
          async () => {
            const { createBookmark: i } =
              await import("./bookmarks-hgxSTBhD.js").then((o) => o.c);
            return { createBookmark: i };
          },
...
async function $e() {
  try {
    return await T("/dashboard/views");
  } catch {
    return [];
  }
}
async function N(e) {
  if (confirm("Delete this view?"))
    try {
      (await T(`/dashboard/views/${e}`, { method: "DELETE" }),
        w("View deleted", "success"),
        document.getElementById("views-dropdown")?.remove());
    } catch (t) {
      w(t.message, "error");
    }
...
async function U(e, t) {
  if (Fe())
    try {
      (await T(`/dashboard/views/${e}/restore`, { method: "POST" }),
        w("View restored!", "success"),
        document.getElementById("views-dropdown")?.remove(),
        we(e),
        be(t || null),
        j(t || null));
      const { loadSettings: s, saveSettings: n } = await f(
        async () => {
          const { loadSettings: a, saveSettings: i } =
            await import("./settings-B0ZpC5LD.js");
          return { loadSettings: a, saveSettings: i };
        },
        __vite__mapDeps([0, 1, 2, 3]),
      );
      (await s(),
        ...
        await n({
          ...
          current_dashboard_view_id: e,
          current_dashboard_view_name: t || null,
        }),
        I(),
        X(),
        Ee());
...
window.saveCurrentView = F;
window.deleteView = N;
window.restoreView = U;
function Ce(e) {
  const t = e.color
    ? `--bookmark-color: ${e.color}; background-color: color-mix(in srgb, ${e.color} 20%, var(--bg-primary)); border-left: 6px solid ${e.color};`
    : "";
  return `
...
function Ie(e, t) {
  const s = t || $.bookmarkSort || "recently_added";
  return [...e].sort((n, a) => {
    switch (s) {
      case "a_z":
      case "a-z":
      ...
      case "z_a":
      case "z-a":
        return a.title.localeCompare(n.title);
      case "most_visited":
        return (a.click_count || 0) - (n.click_count || 0);
      case "oldest_first":
      case "created_asc":
        return (
          new Date(n.created_at).getTime() - new Date(a.created_at).getTime()
        );
      case "recently_added":
      case "created_desc":
      default:
        return (
          new Date(a.created_at).getTime() - new Date(n.created_at).getTime()
        );
    }
  });
}
function I() {
  ...
  const e = document.getElementById("bookmarks-container"),
    ...
    s = document.getElementById("bulk-bar");
  if (!e) return;
  (document.querySelector(".view-toggle")?.classList.add("hidden"),
    s?.classList.add("hidden"),
    (e.className = "dashboard-freeform"),
    (e.innerHTML = ""),
    t && t.classList.add("hidden"),
    Ne());
  const n = document.getElementById("dashboard-views-btn");
  (n && n.classList.remove("hidden"), j(Re));
  const a = `
    ...
    `;
  ((e.innerHTML = a), (e.innerHTML = a), Je(), E(), Ye());
}
function Ye() {
  const e = document.querySelectorAll(".widget-load-more");
  ...
  e.forEach((s) => t.observe(s));
}
function Xe(e) {
  const t = parseInt(e.dataset.widgetIndex || "-1"),
    s = l[t];
  if (!s || e.dataset.loading === "true") return;
  e.dataset.loading = "true";
  const n = Y(s);
  if (!n) return;
  const a = Ie(n.bookmarks, s.sort),
    i = e.parentElement;
  if (!i) return;
  const o = i.querySelectorAll(".compact-item").length,
    r = a.slice(o, o + 20);
  if (r.length > 0) {
    const g = r.map((p) => Ce(p)).join("");
    e.insertAdjacentHTML("beforebegin", g);
  }
  o + r.length >= a.length ? e.remove() : (e.dataset.loading = "false");
}
async function E() {
  try {
    ...
    const t = await T("/tags/analytics");
    let s = [],
      n = [];
    (t &&
      typeof t == "object" &&
      (t.success && Array.isArray(t.tags)
        ? ((s = t.tags), (n = t.cooccurrence || []))
        : Array.isArray(t) && (s = t)),
      e.forEach((a) => {
        const i = a.getAttribute("data-analytics-widget"),
          o = parseInt(i),
          r = (l[o] || {}).settings || {
            metric: "count",
            limit: 20,
          ...
          h = a.querySelector(".tag-analytics-metric"),
          B = a.querySelector(".tag-analytics-limit"),
          D = a.querySelector(".tag-analytics-pairsort"),
          A = a.querySelector(".tag-analytics-color-usage"),
          V = a.querySelector(".tag-analytics-color-clicks"),
          P = a.querySelector(".tag-analytics-color-favorites"),
          q = a.querySelector(".tag-analytics-color-pairs"),
          G = a.querySelector(".legend-usage"),
          J = a.querySelector(".legend-clicks"),
          Z = a.querySelector(".legend-favorites"),
          Q = a.querySelector(".legend-pairs");
        (h && (h.value = g),
          B && (B.value = String(p)),
          D && (D.value = y),
          A && (A.value = v.usage),
          V && (V.value = v.clicks),
          P && (P.value = v.favorites),
          q && (q.value = v.pairs),
          G && (G.style.backgroundColor = v.usage),
          J && (J.style.backgroundColor = v.clicks),
          Z && (Z.style.backgroundColor = v.favorites),
          Q && (Q.style.backgroundColor = v.pairs));
        const K = a.querySelector(".tag-analytics-top-tags"),
          ee = a.querySelector(".tag-analytics-cooccurrence");
        if (K) {
          const c = [...s]
              .sort((b, _) => (_[g] || 0) - (b[g] || 0))
              .slice(0, p),
            u =
              g === "count"
                ? v.usage
        ...
        if (ee) {
          const c = [...n]
            .sort((u, b) => {
              if (y === "alpha") {
                const _ = (u.tag_name_a + " + " + u.tag_name_b).toLowerCase(),
                  x = (b.tag_name_a + " + " + b.tag_name_b).toLowerCase();
                return _.localeCompare(x);
              }
              return (b.count || 0) - (u.count || 0);
            })
            .slice(0, p);
          ee.innerHTML = c
            .map(
        ...
        (h &&
          h.addEventListener("change", (m) => {
            ...
            (l[o].settings || (l[o].settings = {}),
              (l[o].settings.metric = c),
              f(
                async () => {
                  const { saveSettings: u } =
                    await import("./settings-B0ZpC5LD.js");
                  return { saveSettings: u };
                },
                __vite__mapDeps([0, 1, 2, 3]),
              ).then(({ saveSettings: u }) => u({ dashboard_widgets: l })),
              E());
          }),
          B &&
            B.addEventListener("change", (m) => {
              const c = parseInt(m.target.value);
              (l[o].settings || (l[o].settings = {}),
                (l[o].settings.limit = c),
                f(
                  async () => {
                    const { saveSettings: u } =
                      await import("./settings-B0ZpC5LD.js");
        ...
        function z() {
          (l[o].settings || (l[o].settings = {}),
            l[o].settings.colors || (l[o].settings.colors = {}));
        }
        (A &&
          A.addEventListener("change", (m) => {
            (z(),
              (l[o].settings.colors.usage = m.target.value),
              f(
                async () => {
                  const { saveSettings: c } =
                    await import("./settings-B0ZpC5LD.js");
                  return { saveSettings: c };
                },
                __vite__mapDeps([0, 1, 2, 3]),
              ).then(({ saveSettings: c }) => c({ dashboard_widgets: l })),
              E());
          }),
          V &&
            V.addEventListener("change", (m) => {
              (z(),
                (l[o].settings.colors.clicks = m.target.value),
                f(
                  async () => {
                    const { saveSettings: c } =
                      await import("./settings-B0ZpC5LD.js");
          ...
          P &&
            P.addEventListener("change", (m) => {
              (z(),
                (l[o].settings.colors.favorites = m.target.value),
                f(
                  async () => {
                    const { saveSettings: c } =
                      await import("./settings-B0ZpC5LD.js");
          ...
          q &&
            q.addEventListener("change", (m) => {
              (z(),
                (l[o].settings.colors.pairs = m.target.value),
                f(
                  async () => {
                    const { saveSettings: c } =
                      await import("./settings-B0ZpC5LD.js");
...
${b}`;
        ...
        const ae = a.querySelector(".tag-analytics-export-json"),
          oe = a.querySelector(".tag-analytics-export-tags-csv"),
          se = a.querySelector(".tag-analytics-export-pairs-csv");
        (ae &&
          ae.addEventListener("click", () => {
            const m = JSON.stringify({ tags: s, cooccurrence: n }, null, 2);
            R("tag-analytics.json", "application/json", m);
          }),
          oe &&
            oe.addEventListener("click", () => {
              const m = te(s, ["name", "count"]);
              R("tag-analytics-tags.csv", "text/csv", m);
            }),
          se &&
            se.addEventListener("click", () => {
              const m = te(n, ["tag_name_a", "tag_name_b", "count"]);
              R("tag-analytics-pairs.csv", "text/csv", m);
            }));
      }));
  } catch (e) {
    Pe.error("Failed to load tag analytics", e);
  }
}
function Ge() {
  let e = "";
  return (
    l.forEach((t, s) => {
      ...
      const n = Y(t);
      if (!n) return;
      const { name: a, color: i, bookmarks: o, count: d } = n,
        r = Ie(o, t.sort),
        g = t.color || i;
      e += `
  ...
  if (e.type === "folder") {
    ...
    if (pe) {
      const n = (i) => {
          ...
          return (
            de
              .filter((d) => d.parent_id === i)
              .forEach((d) => o.push(...n(d.id))),
            o
          );
        },
        a = n(t.id);
      s = W.filter(
        (i) => i.folder_id && a.includes(i.folder_id) && !i.is_archived,
      );
    } else s = W.filter((n) => n.folder_id === t.id && !n.is_archived);
    return {
      name: t.name,
    ...
    return { name: e.id, color: "#10b981", bookmarks: t, count: t.length };
  } else if (e.type === "tag-analytics")
    return {
      name: "Tag Analytics",
      color: "#6b7280",
      bookmarks: [],
...
function Je() {
  const e = document.getElementById("dashboard-drop-zone");
  if (!e) return;
  (e.addEventListener("dragover", (a) => {
    ...
    e.addEventListener("dragleave", (a) => {
      a.target === e && e.classList.remove("drag-over");
    }),
    e.addEventListener("drop", (a) => {
      (a.preventDefault(), e.classList.remove("drag-over"));
      const i = e.getBoundingClientRect(),
        o = a.clientX - i.left + e.scrollLeft,
        d = a.clientY - i.top + e.scrollTop;
      if (ie) {
        const { type: r, id: g } = ie;
        (Ke(r, g, o, d), Te(null));
      }
    }),
    document.querySelectorAll(".dashboard-widget-freeform").forEach((a) => {
      const i = a.querySelector(".widget-header"),
        o = a.querySelector(".widget-resize-handle");
      (i?.addEventListener("mousedown", (d) => {
        d.target.closest(".widget-remove") ||
          d.target.closest(".widget-color-btn") ||
          (ve(!0),
          he(a),
          ne({ x: d.clientX, y: d.clientY }),
          De({ x: parseInt(a.style.left) || 0, y: parseInt(a.style.top) || 0 }),
        ...
        o?.addEventListener("mousedown", (d) => {
          (ye(!0),
            ...
            ne({ x: d.clientX, y: d.clientY }),
            Ae({
              w: parseInt(a.style.width) || 320,
              h: parseInt(a.style.height) || 400,
            }),
    ...
    document.querySelectorAll('[data-action="remove-widget"]').forEach((a) => {
      a.addEventListener("click", (i) => {
        i.stopPropagation();
        const o = parseInt(a.dataset.index);
        et(o);
      });
    }),
    document
      ...
      .forEach((a) => {
        a.addEventListener("click", (i) => {
          ...
          const d = document.querySelector(
            `.widget-options-container [data-action="toggle-widget-options"][data-index="${o}"]`,
          );
          tt(o, d || a);
        });
      }),
        ...
        a.addEventListener("click", (i) => {
          ...
          const o = a.dataset.index,
            d = document.querySelector(
              `.widget-options-menu[data-widget-index="${o}"]`,
            );
          (document.querySelectorAll(".widget-options-menu").forEach((r) => {
            r !== d && r.classList.add("hidden");
          }),
  ...
  const t = (a) => {
      const i = a.target;
      i.closest(".widget-options-menu") ||
        i.closest('[data-action="toggle-widget-options"]') ||
        i.closest("select") ||
        i.closest("input") ||
        i.closest(".tag-analytics") ||
        document.querySelectorAll(".widget-options-menu").forEach((o) => {
          o.classList.add("hidden");
        });
    },
    s = document.getElementById("dashboard-widgets-freeform");
  (s &&
    !s._menuListenerAdded &&
    (document.addEventListener("click", t), (s._menuListenerAdded = !0)),
    document.querySelectorAll('[data-action="widget-sort-az"]').forEach((a) => {
      a.addEventListener("click", (i) => {
        i.stopPropagation();
        const o = parseInt(a.dataset.widgetIndex);
        l[o] && ((l[o].sort = "a-z"), C(), I());
      });
    }),
    document.querySelectorAll('[data-action="widget-sort-za"]').forEach((a) => {
      a.addEventListener("click", (i) => {
        i.stopPropagation();
        const o = parseInt(a.dataset.widgetIndex);
        l[o] && ((l[o].sort = "z-a"), C(), I());
      });
    }),
    document
      ...
      .forEach((a) => {
        a.addEventListener("click", async (i) => {
          i.stopPropagation();
          const o = a.dataset.widgetType,
            d = a.dataset.widgetId;
          document
            .querySelectorAll(".widget-options-menu")
            .forEach((p) => p.classList.add("hidden"));
          const { openModal: r, resetForms: g } = await f(
            async () => {
              const { openModal: p, resetForms: y } =
                await import("./ui-DJeZlV46.js").then((v) => v.e);
              return { openModal: p, resetForms: y };
            },
            __vite__mapDeps([2, 3, 1]),
          );
          if ((g(), o === "folder")) {
            const p = document.getElementById("bookmark-folder");
            p && (p.value = d);
          } else if (o === "tag") {
            const p = document.getElementById("bookmark-tags");
            p && (p.value = d);
            try {
              const { loadTagsFromInput: y } = await f(
                async () => {
                  const { loadTagsFromInput: v } =
                    await import("./main-BnFaXbWJ.js").then((h) => h.t);
                  return { loadTagsFromInput: v };
                },
                __vite__mapDeps([4, 2, 3, 1, 5, 6]),
              );
              y(d);
            } catch {}
          }
    ...
    document
      ...
      .forEach((a) => {
        a.addEventListener("click", async (i) => {
          ...
          const o = a.dataset.widgetType,
            d = a.dataset.widgetId,
            r = l.find((h) => h.type === o && h.id === d);
          if (!r) {
            (w("Widget not found", "error"),
          ...
          const g = Y(r);
          if (!g) {
            w("Could not retrieve bookmarks", "error");
            return;
          }
          document
            .querySelectorAll(".widget-options-menu")
          ...
          const { bookmarks: p } = g;
          if (!p || p.length === 0) {
            w("No bookmarks to open", "info");
            return;
          }
          if (
            p.length > 5 &&
            !confirm(`Open ${p.length} bookmarks in new tabs?`)
          )
            return;
          let y = 0;
          const v = [];
          (p.forEach((h) => {
            h && h.url && (window.open(h.url, "_blank") ? y++ : v.push(h));
          }),
            y === p.length
              ? w(`Opened ${y} tab${y > 1 ? "s" : ""}`, "success")
              : n(v, y));
        });
      }));
  function n(a, i) {
    let o = document.getElementById("blocked-links-modal");
    if (
      (o ||
        ((o = document.createElement("div")),
        (o.id = "blocked-links-modal"),
        (o.className = "modal-overlay"),
        (o.style.zIndex = "9999"),
        document.body.appendChild(o),
        o.addEventListener("click", (r) => {
          r.target === o && o?.classList.add("hidden");
        })),
      !o)
    )
      return;
  ...
  `),
      !document.getElementById("blocked-links-style"))
    ) {
      const r = document.createElement("style");
      ((r.id = "blocked-links-style"),
        (r.textContent = `
    ...
    .forEach((a) => {
      a.addEventListener("click", async (i) => {
        ...
        if (
          ...
          const { loadBookmarks: r } = await f(
            async () => {
              const { loadBookmarks: g } =
                await import("./bookmarks-hgxSTBhD.js").then((p) => p.c);
              return { loadBookmarks: g };
            },
            __vite__mapDeps([3, 1, 2]),
          );
          (H("folder"), Ve(d), await r());
        } else if (o === "tag") {
          const { loadBookmarks: r, renderBookmarks: g } = await f(
              async () => {
                const { loadBookmarks: y, renderBookmarks: v } =
                  await import("./bookmarks-hgxSTBhD.js").then((h) => h.c);
                return { loadBookmarks: y, renderBookmarks: v };
              },
              __vite__mapDeps([3, 1, 2]),
            ),
            { updateActiveNav: p } = await f(
              async () => {
                const { updateActiveNav: y } =
                  await import("./ui-DJeZlV46.js").then((v) => v.e);
                return { updateActiveNav: y };
              },
              __vite__mapDeps([2, 3, 1]),
            );
          (H("all"), (re.tags = [d]), (re.tagMode = "OR"), await r(), g(), p());
        }
      });
    });
}
function Ze(e) {
  if (ue && S) {
    const t = e.clientX - M.x,
      s = e.clientY - M.y,
      n = le.x + t,
      a = le.y + s;
    ((S.style.left = `${O(n)}px`), (S.style.top = `${O(a)}px`));
  } else if (me && L) {
    const t = e.clientX - M.x,
      s = e.clientY - M.y,
      n = Math.max(150, ce.w + t),
      a = Math.max(100, ce.h + s);
    ((L.style.width = `${O(n)}px`), (L.style.height = `${O(a)}px`));
  }
}
function Qe(e) {
  if (ue && S) {
    S.classList.remove("dragging");
    const t = parseInt(S.dataset.widgetIndex || "-1");
    (l[t] &&
      ((l[t].x = parseInt(S.style.left) || 0),
      (l[t].y = parseInt(S.style.top) || 0),
      C()),
      ve(!1),
      he(null));
  } else if (me && L) {
    L.classList.remove("resizing");
    const t = parseInt(L.dataset.widgetIndex || "-1");
    (l[t] &&
      ((l[t].w = parseInt(L.style.width) || 320),
      (l[t].h = parseInt(L.style.height) || 400),
      C()),
      ye(!1),
      fe(null));
  }
...
function Ke(e, t, s, n) {
  if (l.some((o) => o.type === e && o.id === t)) {
    w("Widget already exists on dashboard", "info");
    return;
  }
  const i = { id: t, type: e, x: s, y: n, w: 320, h: 400 };
  (e === "tag-analytics" &&
    (i.settings = {
      metric: "count",
      limit: 20,
    ...
    _e(),
    X(),
    w(`${e === "folder" ? "Folder" : "Tag"} added to dashboard`, "success"));
}
function et(e) {
  (l.splice(e, 1), C(), I(), _e(), X(), w("Widget removed", "success"));
}
function tt(e, t) {
  const s = document.querySelector(".widget-color-picker");
  s && s.remove();
  const n = l[e];
  if (!n) return;
  const a = [
  ...
  ((i.className = "widget-color-picker"),
    (i.innerHTML = `
  ...
  ((i.style.position = "fixed"),
    (i.style.top = `${o.bottom + 8}px`),
    (i.style.right = `${window.innerWidth - o.right}px`),
    document.body.appendChild(i),
    i.querySelectorAll(".color-picker-option").forEach((d) => {
      d.addEventListener("click", (r) => {
        r.stopPropagation();
        const g = d.dataset.color;
        (at(e, g), i.remove());
      });
    }),
    setTimeout(() => {
      document.addEventListener("click", function d(r) {
        i.contains(r.target) ||
          (i.remove(), document.removeEventListener("click", d));
      });
    }, 100));
}
function at(e, t) {
  l[e] && ((l[e].color = t), C(), I(), w("Widget color updated", "success"));
}
function C() {
  (f(
    async () => {
    ...
    __vite__mapDeps([0, 1, 2, 3]),
  ).then(({ saveSettings: e }) => e({ dashboard_widgets: l })),
    je());
}
function lt(e) {
  const t = document.querySelectorAll(".dashboard-widget"),
    s = e.toLowerCase();
  t.forEach((n) => {
    const a = n.querySelectorAll(".compact-item");
    let i = !1;
    (a.forEach((o) => {
      const r = (
        o.querySelector(".compact-text")?.textContent.toLowerCase() || ""
      ).includes(s);
      ((o.style.display = r || !e ? "" : "none"), (r || !e) && (i = !0));
    }),
      (n.style.opacity = i || !e ? "1" : "0.5"));
  });
}
function X() {
  const e = document.getElementById("layout-stats-content");
  if (e) {
    const t = l.length;
    e.innerHTML = `<strong>${t}</strong> widget${t !== 1 ? "s" : ""} on dashboard`;
  }
}
window.saveCurrentView = F;
window.deleteView = N;
window.restoreView = U;
export {
  Ke as addDashboardWidget,
  Fe as confirmViewSwitch,
  N as deleteView,
  lt as filterDashboardBookmarks,
  Je as initDashboardDragDrop,
  Ne as initDashboardViews,
  E as initTagAnalyticsWidgets,
  je as markDashboardModified,
  et as removeDashboardWidget,
  I as renderDashboard,
  U as restoreView,
  F as saveCurrentView,
  Ee as saveDashboardStateSnapshot,
  dt as toggleFullscreen,
  X as updateLayoutStats,
  j as updateViewNameBadge,
};
