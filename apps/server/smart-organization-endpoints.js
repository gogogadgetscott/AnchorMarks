 ...
 *
 * Add these endpoints to server/app.js
 *
 * Usage:
 * 1. Add to top of app.js after other requires:
 *    const smartOrg = require('./smart-organization');
 *
 * 2. Add all the endpoints below to app.js after other app.get/app.post routes
 */
...
app.get("/api/smart-insights", authenticateToken, (req, res) => {
  try {
    ...
    // Suggestions
    const suggestedCollections = smartOrg
      .getActivityCollections(db, req.user.id)
      .slice(0, 2);
    const suggestedClusters = smartOrg
      .getTagClusters(db, req.user.id)
      .slice(0, 2);
