...
export function getCommandPaletteCommands() {
    const baseCommands = [
        ...
        { label: 'Focus search', action: () => document.getElementById('search-input')?.focus() },
        {
...
export function renderCommandPaletteList(filterText) {
    ...
    const entries = getCommandPaletteCommands().filter(cmd => cmd.label.toLowerCase().includes(term));
    ...
    list.innerHTML = entries.map((cmd, idx) => `
...
export function updateCommandPaletteActive(direction) {
    ...
    const newIndex = Math.max(0, Math.min(state.commandPaletteEntries.length - 1, state.commandPaletteActiveIndex + direction));
