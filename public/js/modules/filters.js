...
export async function showFilterDropdown() {
    ...
    try {
        ...
        if (bookmarksHeader && bookmarksHeader.style.display !== 'none') {
            ...
            console.log('Dropdown in DOM:', document.getElementById('filter-dropdown'));
        ...
        if (sortSelect) sortSelect.value = state.filterConfig.sort || 'recently_added';
...
function getFolderBookmarkCount(folderId) {
    ...
    return state.bookmarks.filter(b => descendantIds.includes(b.folder_id)).length;
}
...
async function renderFoldersInDropdown() {
    ...
    // No Folder
    if (noFolderCount > 0) {
        const isNoFolderActive = !state.currentFolder && state.currentView === 'folder';
        html += `
    ...
    container.innerHTML = html || '<p style="color:var(--text-tertiary);font-size:0.875rem;padding:1rem;">No folders</p>';
...
function attachFilterDropdownListeners() {
    ...
    if (pinBtn) {
        pinBtn.addEventListener('click', (e) => {
            ...
            if (svg) {
                svg.setAttribute('fill', filterDropdownPinned ? 'currentColor' : 'none');
            }
            ...
            showToast(`Filters ${filterDropdownPinned ? 'pinned' : 'unpinned'}`, 'success');
        });
    }
